    ...
    ui(new Ui::SettingsMultisendWidget)
{
    ...
    ui->labelSubtitle1->setText(tr("MultiSend allows you to automatically send up to 100% of your stake or masternode reward to a list of other BTCc addresses after it matures."));
...
void SettingsMultisendWidget::activate()
{
    ...
    } else if (IsValidDestinationString(pwalletMain->vMultiSend[0].first, Params())) {
        pwalletMain->fMultiSendStake = ui->checkBoxStake->isChecked();
        pwalletMain->fMultiSendMasternodeReward = ui->checkBoxRewards->isChecked();
