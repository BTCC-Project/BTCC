...
const char * const PIVX_CONF_FILENAME = "btcc.conf";
const char * const PIVX_PID_FILENAME = "btcc.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\btcc
// Windows >= Vista: C:\Users\Username\AppData\Roaming\btcc
// Mac: ~/Library/Application Support/btcc
// Unix: ~/.btcc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTCc";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty btcc.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override btcc.conf
