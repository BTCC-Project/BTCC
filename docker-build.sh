...
if [ $# -ne 6 ]; then
    ...
    echo "Usage: $0 BTCC __BTCc__ __forking-altcoins__ develop 3.0.0.0 Linux"
    exit 1
fi
...
docker build \
	...
	-f ./contrib/docker/Dockerfile.btcc-${ARCHITECTURE2}-wallet \
...
if [ "$TICKER" == "BTCC" ]; then
	...
	response=$(curl -s https://api.github.com/repos/forking-altcoins/${TICKER}/releases/latest)
